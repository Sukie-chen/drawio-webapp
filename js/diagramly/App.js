...
App = function(editor, container, lightbox)
{
	...
	// 			if (file.constructor == window.DriveFile && file.desc != null && this.drive != null)
	// 			{
	...
	{
		var file = this.getCurrentFile();
		console.log('autosaveChanged file', file);
...
App.TRELLO_JQUERY_URL = './js/jq/jquery-1.11.3.min.js';
	...
	function doLoad(bundle)
	{
			...
			STYLE_PATH + '/default.xml', STYLE_PATH + '/dark-default.xml'], function(xhr)
		{
			...
			if (window.mxscript != null)
			{
				...
				// // Disables client
				// else 
				if (typeof window.Dropbox === 'undefined' || typeof window.Dropbox.choose === 'undefined')
				{
					window.DropboxClient = null;
				}
...
if (urlParams['embed'] != '1')
{
	App.prototype.menubarHeight = !window.isIframe ? 64:30;
}
else
{
...
App.prototype.init = function()
{
	...
	// 		urlParams['gh'] == '1')) ? new window.GitHubClient(this) : null;
	...
	// 	urlParams['gl'] == '1')) ? new window.GitLabClient(this) : null;
	...
	// 			 */
	// 			this.oneDrive = new window.OneDriveClient(this);
	...
	// 			try
	// 			{
	// 				this.trello = new window.TrelloClient(this);
	...
	// 			{
	// 				this.drive = new window.DriveClient(this);
	...
	// 			try
	// 			{
	// 				this.dropbox = new window.DropboxClient(this);
...
App.prototype.appIconClicked = function(evt)
{
	...
	else
	{
		...
		{
			if (file != null && file.constructor == window.GitHubFile)
			{
		...
		{
			if (file != null && file.constructor == window.GitLabFile)
			{
				this.openLink(file.meta.html_url);
			}
			else
...
App.prototype.getLibraryStorageHint = function(file)
{
	...
	// if (file.constructor == window.DriveLibrary)
	// {
	// 	tip += ' (' + mxResources.get('googleDrive') + ')';
	// }
	// else if (file.constructor == window.GitHubLibrary)
	// {
	// 	tip += ' (' + mxResources.get('github') + ')';
	// }
	// else if (file.constructor == window.TrelloLibrary)
	// {
	// 	tip += ' (' + mxResources.get('trello') + ')';
	// }
	// else if (file.constructor == window.DropboxLibrary)
	// {
	// 	tip += ' (' + mxResources.get('dropbox') + ')';
	// }
	// else if (file.constructor == window.OneDriveLibrary)
	// {
...
App.prototype.pickFolder = function(mode, fn, enabled, direct, force)
{
	...
	else if (enabled && mode == App.MODE_ONEDRIVE && this.oneDrive != null)
	{
		this.oneDrive.pickFolder(mxUtils.bind(this, function(files)
		{
			...
			{
				folderId = window.OneDriveFile.prototype.getIdOf(files.value[0]);
        		fn(folderId);
			}
		}), direct);
	}
...
App.prototype.updateHeader = function()
{
	if (this.menubar != null)
	{
		...
		if(!window.isIframe) {
			...
			this.fnameWrapper.style.lineHeight = '26px';
			...
			this.fname.style.fontSize = '16px';//'18px';
		...
		this.toggleFormatElement.style.top = 0;// (uiTheme == 'atlas') ? '8px' : '6px';
		// this.toggleFormatElement.style.right = (uiTheme != 'atlas' && urlParams['embed'] != '1' && !window.isIframe) ? '30px' : '10px';
		this.toggleFormatElement.style.right = (uiTheme != 'atlas' && urlParams['embed'] != '1' && !window.isIframe) ? '40px' : '10px';
		...
		this.toggleFormatElement.className = 'geButton';
		this.toggleFormatElement.style.width = '20px';//'16px';
		this.toggleFormatElement.style.height = '20px'; //'16px';
		...
		this.fullscreenElement.style.top = 0;//(uiTheme == 'atlas') ? '8px' : '6px';
		// this.fullscreenElement.style.right = (uiTheme != 'atlas' && urlParams['embed'] != '1' && !window.isIframe) ? '50px' : '30px';
		this.fullscreenElement.style.right = (uiTheme != 'atlas' && urlParams['embed'] != '1' && !window.isIframe) ? '70px' : '40px';
		...
		this.fullscreenElement.style.width = '20px';//'16px';
		this.fullscreenElement.style.height = '20px'; //'16px';
		...
		if (urlParams['embed'] != '1')
		{
			...
			if(!window.isIframe) {
				...
				this.toggleElement.style.width = '20px';//'16px';
				this.toggleElement.style.height = '20px';//'16px';
				this.toggleElement.style.color = '#666';
				this.toggleElement.style.top = 0; // (uiTheme == 'atlas') ? '8px' : '6px';
					...
					if (this.drive != null)
					{
						...
						if (driveUsers.length > 0)
						{
							...
							var closeFile = mxUtils.bind(this, function(callback, spinnerMsg)
							{
								...
								if (file != null && file.constructor == window.DriveFile)
								{
...
//									file.close();
					...
					if (this.dropbox != null)
					{
						addUser(this.dropbox.getUser(), IMAGE_PATH + '/dropbox-logo.svg', mxUtils.bind(this, function()
						{
							...
							if (file != null && file.constructor == window.DropboxFile)
							{
					...
					if (this.oneDrive != null)
					{
						addUser(this.oneDrive.getUser(), IMAGE_PATH + '/onedrive-logo.svg', mxUtils.bind(this, function()
						{
							...
							if (file != null && file.constructor == window.OneDriveFile)
							{
								var doLogout = mxUtils.bind(this, function()
								{
					...
					if (this.trello != null)
					{
						addUser(this.trello.getUser(), IMAGE_PATH + '/trello-logo.svg', mxUtils.bind(this, function()
						{
							...
							if (file != null && file.constructor == window.TrelloFile)
							{
								var doLogout = mxUtils.bind(this, function()
								{
									this.trello.logout();
